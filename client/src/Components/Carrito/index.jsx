      ...
      >
        <div className="modal-content" style={{ zIndex: '999999999' }}>
              ...
              arrayCarrito?.map((ele, index) => {
                return (
                  <div key={index} className='cartitasCarrito'>
                    ...
                    <h5 className="precioApartado">{`$${ele.cantidad * parseInt(ele.price)}`}</h5>
          ...
          <div className="modal-footer" style={{ position: 'relative' }}>
            <p style={{ position: 'absolute', left: '20px', bottom: '0%', color: '#000' }}>{arrayCarrito?.reduce((ant, des) => ant + parseInt(des.price) * des.cantidad, 0)}</p>
